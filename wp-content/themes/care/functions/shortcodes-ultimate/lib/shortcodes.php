...
$img_style ='';
	...
	function su_callout_shortcode( $atts, $content = null ) {
		...
		return '<div class="su-callout" style="background-color:' . $bg . '; color:' . $color . ';">' . do_shortcode( $content ) . '</div>';
	}
	...
	function su_jcarousel_shortcode( $atts, $content = null ) {
		...
		// No attachments
			...
			$return = '<p class="su-error"><strong>jCarousel:</strong> ' . __( 'no attached images, or only one attached image', 'care' ) . '&hellip;</p>';
		}
	...
	function su_blog_shortcode( $atts, $content = null ) {
     ...
     global $post;
		...
		foreach($myposts as $post) :
				...
				// Background no
				if ( $background == 'no' ) {
					$return .= '<div class="su-posts-'. $style .'"><h2 style="font-size:'.$titlesize.'px;"><a href="'.get_permalink().'">'.the_title("","",false).'</a></h2>';
				}
				// Background yes
				elseif ( $background == 'yes' ) {
					$return .= '<div class="su-posts-'. $style .' su-posts-bg" style="background:'.$bg_color.';"><h2 style="font-size:'.$titlesize.'px;"><a href="'.get_permalink().'">'.the_title("","",false).'</a></h2>';
				}

				// Meta
				if ( $meta == 'yes' ) {
					$return .= '<div class="entry-utility">';
					$return .= '<span class="entry-date"><img src="'.get_bloginfo('template_url') . '/images/date-ico.png" /><a href="'.get_permalink(). '">'. get_the_date(). '</a></span><span class="author vcard"> <img src="'.get_bloginfo('template_url') . '/images/author-ico.png" /><a href="'. get_author_posts_url( get_the_author_meta( 'ID' ) ) . '">' . get_the_author().'</a></span><span class="cat-links"><img src="'.get_bloginfo('template_url') .'/images/folder-ico.png" />
					'. get_the_category_list( ', ' ).'</span><img src="'.get_bloginfo('template_url') .'/images/comment-ico.png" />';
					if ( comments_open() ){
						  if($num_comments == 0){

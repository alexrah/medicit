<?php
	function su_plugin_init() {
		...
		wp_register_script( 'flowplayer', su_plugin_url() . '/js/flowplayer-3.2.6.min.js', false, su_get_version(), false );
		...
		if ( !is_admin() ) {
			...
			if ( !isset( $disabled_scripts['flowplayer'] ) ) {
		...
		elseif ( is_admin() ) {
			...
			// Pages for including
			$su_generator_includes_pages = array( 'post.php', 'edit.php', 'post-new.php', 'index.php', 'page.php', 'single.php' );
	...
	function su_plugin_url() {
		...
		return get_bloginfo('template_url') . '/functions/shortcodes-ultimate';
	}
	...
	function su_add_settings_link( $links ) {
		$links[] = '<a href="' . admin_url( 'options-general.php?page=shortcodes-ultimate' ) . '">' . __( 'Settings', 'care' ) . '</a>';
		return $links;
	}
	...
	function su_save_notification() {
		...
		if ( isset( $_POST['save'] ) && $_GET['page'] == 'shortcodes-ultimate' ) {
			echo '<div class="updated"><p><strong>' . __( 'Settings saved', 'care' ) . '</strong></p></div>';
		}

		// Save custom css
		if ( isset( $_POST['save-css'] ) && $_GET['page'] == 'shortcodes-ultimate' ) {
			echo '<div class="updated"><p><strong>' . __( 'Custom CSS saved', 'care' ) . '</strong></p></div>';
		}
	}
	...
	function su_add_generator_button() {
		echo '<a href="#TB_inline?width=640&height=800&inlineId=su-generator-wrap" class="thickbox" title="' . __( 'Insert shortcode', 'care' ) . '"><img src="' . su_plugin_url() . '/images/admin/media-icon.png" alt="" /></a>';
	}
				...
				<div id="su-generator-shell">
					<div id="su-generator-header">
						<select id="su-generator-select">
							<option value="raw"><?php _e( 'Select shortcode', 'care' ); ?></option>
							<?php
						...
						<div id="su-generator-tools">
							<!-- <a href="<?php echo admin_url( 'options-general.php?page=shortcodes-ultimate' ); ?>" target="_blank" title="<?php _e( 'Settings', 'care' ); ?>"><img src="<?php echo su_plugin_url(); ?>/images/generator/settings.png" alt="" /></a> -->
							<a href="http://www.colorpicker.com/" target="_blank" title="<?php _e( 'Color picker', 'care' ); ?>"><img src="<?php echo su_plugin_url(); ?>/images/generator/colorpicker.png" alt="" /></a>
							<!--<a href="http://wordpress.org/tags/shortcodes-ultimate?forum_id=10" target="_blank" title="<?php _e( 'Support forum', 'care' ); ?>"><img src="<?php echo su_plugin_url(); ?>/images/generator/support.png" alt="" /></a> -->
						</div>
					</div>

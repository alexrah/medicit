...
define('GRUNION_PLUGIN_DIR', TEMPLATEPATH . '/functions/contact-form');
define('GRUNION_PLUGIN_URL', get_template_directory_uri() . '/functions/contact-form');
...
global $contact_form_last_id, $contact_form_errors, $contact_form_fields, $current_user, $user_identity;
	...
	if ( $field['type'] == 'email' ) {
		$r .= "\n<div>\n";
		$r .= "\t\t<label for='".esc_attr($field_id)."' class='lb_heading grunion-field-label ".esc_attr($field['type']) . ( contact_form_is_error($field_id) ? ' form-error' : '' ) . "'>" . htmlspecialchars( $field['label'] ) . ( $field['required'] ? '<span> '. __($contact_required) . '</span>' : '' ) . "</label>\n";
		...
		$r .= "\n<div>\n";
		$r .= "\t\t<label for='".esc_attr($field_id)."' class='lb_heading ".esc_attr($field['type']) . ( contact_form_is_error($field_id) ? ' form-error' : '' ) . "'>" . htmlspecialchars( $field['label'] ) . ( $field['required'] ? '<span> '. __($contact_required) . '</span>' : '' ) . "</label>";$r .= "\t\t<textarea name='".esc_attr($field_id)."' id='".esc_attr($field_id)."' rows='20' cols='20'>".htmlspecialchars($field_value)."</textarea>\n";
		$r .= "\t</div>\n";
		...
		$r .= "\t<div><label class='lb_heading ". ( contact_form_is_error($field_id) ? ' form-error' : '' ) . "'>" . htmlspecialchars( $field['label'] ) . ( $field['required'] ? '<span> '. __($contact_required) . '</span>' : '' ) . "</label>\n";
		...
		$r .= "\n<div>\n";
		$r .= "\t\t<label for='".esc_attr($field_id)."' class='lb_heading ".esc_attr($field['type']) . ( contact_form_is_error($field_id) ? ' form-error' : '' ) . "'>" . htmlspecialchars( $field['label'] ) . ( $field['required'] ? '<span> '. __($contact_required) . '</span>' : '' ) . "</label>\n";
...
function contact_form_validate_field( $field ) {
	...
	if ( $field['required'] && !trim($field_value) ) {
		...
		$contact_form_errors->add( $field_id, sprintf( __('%s <span style="color:#ff0000;">'.$contact_is_required.'</span>'), $field['label'] ) );
	}
...
*/
	...
	$subject = apply_filters( 'contact_form_subject', isset($spam) && $spam . $contact_form_subject );
...
";
	...
	update_post_meta( $post_id, '_feedback_akismet_values', wp_kses( isset($akismet_values) && $akismet_values , array() ) );
